...
const resistances = [
    0, 10, 20, 30, 60, 90, 120, 280, 360, 480, 600, 1000, 2000, 3000, '∞'
];

const ticks = [
    25, 23, 22, 21, 18, 15, 13, 9, 8, 6, 5, 3, 2, 1, 0
];
...
const drawDial = () => {
    ...
    const angles = ticks.map(tick => (tick / 25) * (2 / 3) * Math.PI - (Math.PI * 5 / 6));
    ...
    angles.forEach((angle, index) => {
        ...
        const labelX = centerX + (radius - 40) * Math.cos(angle); // 调整距离
        const labelY = centerY + (radius - 40) * Math.sin(angle); // 调整距离
        ctx.font = "16px Arial";
        ctx.fillStyle = "black";
